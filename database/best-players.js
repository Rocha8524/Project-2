...
function json2table(json, $card) {
  ...
  $card.html("<thead><tr></tr></thead><tbody></tbody>");
  ...
  $card.find("thead tr").append(headerRow);
  $card.find("tbody").append(bodyRows);
}

dom.$data.val(JSON.stringify(players));
json2table(players, dom.$card);

dom.$data.on("input", function() {
  json2table(JSON.parse(dom.$data.val()), dom.$card);
});
*/
